    ...
    for i in 1:length(sizes)
        for j in 1:setNumbers
            ...
            if benchmarking
                ...
                bm = run(@benchmarkable callQHull($points) samples=30 seconds=10000)
                # create report ignoring the worst 10 runs
                runningTime[k,:] = report(bm, 10)
            else
                exportFile = string(resultDirectory, instanceNames[k], "_QHull")
                callQHull(points, exportResult, exportFile)
            end
        end
    end

    if benchmarking
        exportReport(instanceNames, runningTime, string(resultDirectory,"qHull_running_time.csv"))
    end
end
